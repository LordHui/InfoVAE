...
def conv_generator(z, data_dims, range):
    with tf.variable_scope('g_net') as vs:
        ...
        xstddev = tf.contrib.layers.convolution2d_transpose(conv1, data_dims[-1], 4, 2, activation_fn=tf.sigmoid)
        xmean = xmean * (range[1] - range[0]) + range[0]
        xstddev += 0.001
...
class GenerativeAdversarialNet(object):
    def __init__(self, dataset, name="gan"):
        ...
        self.x_mean, self.x_stddev = generator(self.z, data_dims=self.data_dims, range=dataset.range)
        self.x_sample = self.x_mean + tf.multiply(self.x_stddev,
                                             tf.random_normal(tf.stack([tf.shape(self.z)[0]] + self.data_dims)))
        ...
        self.x_recon = 0.1 * tf.reduce_sum(tf.log(self.x_stddev) + tf.square(self.x - self.x_mean) / tf.square(self.x_stddev) / 2, axis=1)
